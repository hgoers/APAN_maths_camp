---
title: "Session 1: Data and Measurement"
format:
  html:
    code-link: true
toc-depth: 4
...
-   We have information on `nrow(nes) |> comma()` observations (or respondents)
...
What count and proportion of respondents fall into each age bracket the survey recorded? We can take advantage of `janitor::tabyl()` to quickly calculate this:
...
From this we learn that most countries spend between three to five percent of their GDP on education. There appears to be an **outlier**. A country or group of countries spend over 10 percent of their GDP on education.
...
From this we learn that most countries spend around four percent of their GDP on education. There is a small cluster of countries that spend between around 7.5 to nine percent on these services. Three countries spend an unusually large proportion of their GDP (over 10 percent of it) on education annually. 
...
::: {.callout-tip}
If you have an even number of observations, the median is the mid-point between the two middle numbers of your ordered values. For example, the median of all values from one to 10 is 5.5.
:::
...
We can find the modal region in our dataset using base R's `table()`: 
